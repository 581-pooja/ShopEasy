// viewControllers/useHomeViewController.ts
...
export const useHomeViewController = () => {
  ...
  const onGenderOpen = useCallback(() => {
    // noop placeholder like original
  }, []);
  ...
  useEffect(() => {
    if (vm.userId) vm.fetchAddresses();
    // re-fetch addresses when returning from Address screen if sheet was open
  }, [vm.userId, modalVisible]);
  ...
  const filteredProducts = vm.products?.filter((p: any) => p.category === category) || [];
